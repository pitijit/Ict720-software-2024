...
def on_message(client, userdata, msg):
    logging.info('Received message: %s from %s', msg.payload, msg.topic)
    dev_db = mongo_client.dev_db
    dev_col = dev_db.devices  # database 1
    dev_evts = dev_db.dev_events  # database 2
    # extract dev_id from its corresponding topic; taist/aiot/sound/dev_00 for example
    dev_id = msg.topic.split('/')[-1]
    # get the event ; ; taist/aiot/sound/dev_00 for example
    evt_type = msg.topic.split('/')[-2]
    if evt_type == 'sound':  # concern only sound event; ; taist/aiot/sound/dev_00 for example
        dev_doc = dev_col.find_one({'dev_id': dev_id})
        if dev_doc is not None:  # if this device is already registered in our database
